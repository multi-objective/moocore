...
def generate_ndset(
    ...
    * ``"convex-simplex"``: Equivalent to ``generate_ndset(...,
      method="simplex") ** 2``, which is convex for minimisation problems. Such
      a set cannot be obtained by any affine transformation of a subset of the
      hyper-sphere.  This sampling is *not* uniform.

    * ``"convex-sphere"`` or ``"X"``: Equivalent to ``1 - generate_ndset(...,
      method="concave-sphere")``, which is convex for minimisation problems.
      This shape has also been called *inverted convex*
      :footcite:p:`IshHeSha2019regular`. This sampling is uniform.
    ...
    * ``"concave-simplex"``: Equivalent to ``1 - generate_ndset(...,
      method="convex-simplex")``, which is concave for minimisation problems.
      This shape has also been called *inverted concave*
      :footcite:p:`IshHeSha2019regular`.  This sampling is *not* uniform
      because ``method="convex-simplex"`` is also not uniform.


    Method ``"simplex"`` uniformly samples points on the standard
    :math:`(d-1)`-simplex defined by :math:`\{\vec{x}\in \mathbb{R}_+^d :
    \sum_{i=1}^d x_i = 1\}`.  This shape of nondominated set is also called
    ``"linear"`` in the literature :footcite:p:`LacKlaFon2017box`.  Each point
    :math:`\vec{z} \in (0,1)^d \subset \mathbb{R}^d` is generated by sampling
    :math:`d` independent and identically distributed values :math:`(x_1,x_2,
    \dots, x_d)` from the exponential distribution, then dividing each value by
    the l1-norm of the vector, :math:`z_i = x_i / \sum_{i=1}^d x_i`
    ...
    :footcite:p:`IshHeSha2019regular`.
