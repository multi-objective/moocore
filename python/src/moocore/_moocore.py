...
def hv_approx(
    ...
    *,
    ...
    if (
        not is_integer_value(nsamples)
        or nsamples <= 0
        or nsamples >= 2147483648
    ):
        raise ValueError(
...
def hv_approx_fpras(
    ...
    *,
    ...
    :footcite:t:`BriFri2010approx`.  This algorithm returns, with probability
    ...
    The algorithm requires :math:`O(\frac{nm}{\epsilon^2}\log\frac{1}{\delta})`. That is,
    ...
    discrete probability distribution :footcite:p:`Vose1991alias`, which
    requires :math:`O(1)` per sample.  Using the naive roulette-wheel method
    would add, at least, a factor of :math:`O(\log n)` to the above runtime.
    ...
    if hv < 0:
        raise ValueError(
            f"The requested approximation (epsilon={epsilon}, delta={delta}) would require a very long time"
        )
    return hv
