...
def generate_ndset(
    ...
    * ``"convex-simplex"``: Equivalent to ``generate_ndset("concave-sphere", n,
    ...
    is also called *linear* in the literature :footcite:p:`LacKlaFon2017box`.
    Points are generated by the method given by
    :footcite:t:`RubMel1998simulation`.  That is, each point :math:`\vec{z} \in
    ...
    l1-norm of the vector, :math:`z_i = x_i / \sum_{i=1}^d x_i`.  Values
    ...
    minimised. Points are generated by the method proposed by
    :footcite:t:`Muller1959sphere`.  That is, each point :math:`\vec{z} \in
    (0,1)^d \subset \mathbb{R}^d` is generated by sampling :math:`d`
    independent and identically distributed values :math:`\vec{x}=(x_1,x_2,
    \dots, x_d)` from the standard normal distribution, then dividing each
    value by the l2-norm of the vector, :math:`z_i =
    \frac{|x_i|}{\|\vec{x}\|_2}`. The absolute value in the numerator ensures
    ...
    Methods ``"simplex"``, ``"concave-sphere"`` and ``"convex-sphere"`` are
    different from the similar ones suggested by
    :footcite:t:`LacKlaFon2017box`.  The latter methods sample values from a
    uniform distribution before projecting the values to a simplex or
    hyper-sphere, which does not result in a uniform sampling on the projection
    surface.
