...
- `is_nondominated` is up to 10x faster in some inputs thanks to a
  customized radixsort implementation.
- New shapes `"cliff-concave"` and `"cliff-convex"` added to
...
- Fix crash in
  ...
  with duplicated points
  ([\#74](https://github.com/multi-objective/moocore/issues/74)).
- The maximum number of dimensions supported by most functions is now
  raised to 255, except for hypervolume and hypervolume approximation,
  where it remains 31. Functions now give a clear error to the user when
  the limits are reached
  ([\#72](https://github.com/multi-objective/moocore/issues/72))
...
- Add `GPGame` and `targeted` packages to
...
- [`is_nondominated()`](https://multi-objective.github.io/moocore/r/reference/nondominated.md),
  ...
  and
  [`pareto_rank()`](https://multi-objective.github.io/moocore/r/reference/pareto_rank.md)
  use a significantly faster algorithm by Kung et al. for dimensions
  larger than 3.
- [`generate_ndset()`](https://multi-objective.github.io/moocore/r/reference/generate_ndset.md):
  New shapes `"inverted-simplex"` and `"concave-simplex"`. Shape
  `"convex-simplex"` is now equivalent to
  `generate_ndset(..., method="simplex")^2`, which is slightly more
...
- The default method of
  ...
  is now `"Rphi-FWE+"`, which is typically as accurate as the other
  methods, but significantly faster.
