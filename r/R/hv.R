...
#' \citep{GueFon2017hv4d} as the base case, resulting in a \eqn{O(n^{d-2})}
#' time complexity and \eqn{O(n)} space complexity in the worst-case, where
#' \eqn{d} is the dimension of the points.  Experimental results show that the
#' pruning techniques used may reduce the time complexity even further.  The
#' original proposal \citep{FonPaqLop06:hypervolume} had the HV3D algorithm as
#' the base case, giving a time complexity of \eqn{O(n^{d-2} \log n)}.  Andreia
