...
dataset using both `whv_hype()` (HypE), and `hv_approx()` for several
values of the number of samples between $10^1$ and $10^5$.  We repeat each
calculation 5 times to account for stochasticity.
...
```{r hype_dz2019}
#| fig.alt = "Comparison of HypE and Rphi-FWE+ with increasing number of samples."
library(ggplot2)
library(scales)
...
The quasi-Monte-Carlo approximations with `method=Rphi-FWE+` [@Lop2026hvapprox]
and `method=DZ2019-HW` [@DenZha2019approxhv] are deterministic, but not
monotonic on the number of samples. Nevertheless, they are often better than
the Monte-Carlo approximation generated with `method=DZ2019-MC`
[@DenZha2019approxhv], specially with large number of objectives.  A more
detailed comparison is provided by @Lop2026hvapprox.
...
`hv_approx_fpras()` allows obtaining an approximation with relative error
smaller than $\epsilon$ (`epsilon`) with a given probability $1-\delta$
(`delta`).  As the plot below shows, the approximation error is often better
than the requested value, but the computation time increases very quickly for
smaller `epsilon`.

```{r fpras}
#| fig.alt = "Boxplot of relative error and line plot of CPU time for FPRAS with varying epsilon and delta."
library(ggplot2)
library(scales)
shape <- "convex-sphere"
ref <- 1.1
