...
When `ncol(data) == 2`, the code uses the \\O(n \log n)\\ algorithm by
Jensen (2003) . When `ncol(data) == 3`, it uses a \\O(k\cdot n \log n)\\
algorithm, where \\k\\ is the number of fronts in the output. With
higher dimensions, it uses the naive \\O(n^3)\\ algorithm.
