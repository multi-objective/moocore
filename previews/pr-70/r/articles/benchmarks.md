...
[`targeted`](https://cran.r-project.org/package=targeted) and
[`caRamel`](https://cran.r-project.org/package=caRamel). Other R
packages are not included in the comparison because they are based on
these packages for the functionality benchmarked, so they are **at least
as slow** as them. For example,
[`GPareto`](http://cran.r-project.org/package=GPareto) and
[`rmoo`](https://cran.r-project.org/package=rmoo) use
[`emoa`](https://cran.r-project.org/package=emoa) to compute the
hypervolume. [`bbotk`](https://cran.r-project.org/package=bbotk) already
uses [`moocore`](https://multi-objective.github.io/moocore/r/). Not all
packages provide the same functionality.
...
benchmark <- function(name, x, N, setup, expr.list, prefix, title, check)
{
  ...
  if (run_benchmarks || !file.exists(rds_file)) {
    ...
    if (!requireNamespace("atime", quietly = TRUE)
      || packageVersion("atime") < "2026.7.1") {
      stop("Please install atime >= 2026.7.1")
    }
...
benchmark_all <- function(files, prefix, title, setup, expr.list, filter, check=TRUE)
{
  for (name in names(files)) {
    p <- benchmark(name = name, x = get_ndset(files[[name]], filter=filter),
      N = files[[name]]$N, prefix = prefix, title = title,
      setup = setup,  expr.list = expr.list, check=check)
    print(p)
  }
}
```
...
in 2D, 3D, 4D, 5D and 10D. The plots show that
[`moocore`](https://multi-objective.github.io/moocore/r/) is always 10
times faster than [`GPGame`](https://cran.r-project.org/package=GPGame),
[`caRamel`](https://cran.r-project.org/package=caRamel) and
[`targeted`](https://cran.r-project.org/package=targeted) for any number
of objectives. In addition,
[`GPGame`](https://cran.r-project.org/package=GPGame) calculates wrong
values with repeated coordinates
([vpicheny/GPGame#2](https://github.com/vpicheny/GPGame/issues/2)). The
`rand4d` testcase is somewhat different from the others, as it is mostly
composed of dominated points (only 10590 points are nondominated), while
the other testcases are mostly nondominated sets.
...
expr.list <- list(
  ...
  caRamel = quote(caRamel::pareto(nz))
)
...
files <- list(
  ...
  "convex-3d"=list(generate=list(25000, 3, "convex-sphere", 42), N=geomspace(1000, 25000, 10)),
  "sphere-3d"=list(generate=list(25000, 3, "sphere", 42), N=geomspace(1000, 25000, 10)),
...
benchmark_all(files, prefix="ndom", title = "is_(non)dominated()",
  setup = setup, expr.list = expr.list, filter=FALSE, check=FALSE)
```

![](benchmarks_files/figure-html/ndom-1.png)![](benchmarks_files/figure-html/ndom-2.png)![](benchmarks_files/figure-html/ndom-3.png)![](benchmarks_files/figure-html/ndom-4.png)![](benchmarks_files/figure-html/ndom-5.png)![](benchmarks_files/figure-html/ndom-6.png)![](benchmarks_files/figure-html/ndom-7.png)![](benchmarks_files/figure-html/ndom-8.png)
...
files <- list(
  "spherical-3d"=list(
    file = "spherical-3d-2000pts-10.dat.xz",
    N = geomspace(100, 20000, 10)),
