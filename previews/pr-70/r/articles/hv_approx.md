...
for several values of the number of samples between $`10^1`$ and
$`10^5`$. We repeat each calculation 5 times to account for
stochasticity.
...
10^5\}`$ random samples to show the higher samples reduce the
approximation error. Since the approximation is stochastic, we perform 5
repetitions of each computation.
...
nsamples_exp <- 5
set.seed(42)
results <- list("HypE" = list(), "DZ2019-HW" = list(), "DZ2019-MC" = list(), "Rphi-FWE+" = list())
...
for (i in seq_len(nsamples_exp)) {
  df <- rbind(df,
    ...
    data.frame(Method="Rphi-FWE+", rep=c(1,1), samples=10^i,
      value = hv_approx(x, reference = ref, nsamples = 10^i, method="Rphi-FWE+")),
...
#> Mean of DZ2019-MC : 1.057233221522  [min, max] = [1.056868277766, 1.057665468167]
#>      Mean of HypE : 1.058779259998  [min, max] = [1.055621699998, 1.063736099998]
#> Mean of Rphi-FWE+ : 1.057042456377  [min, max] = [1.057042456377, 1.057042456377]
```
...
![Comparison of HypE and Rphi-FWE+ with increasing number of
...
([López-Ibáñez 2026](#ref-Lop2026hvapprox)) and `method=DZ2019-HW`
([Deng and Zhang 2019](#ref-DenZha2019approxhv)) are deterministic, but
not monotonic on the number of samples. Nevertheless, they are often
better than the Monte-Carlo approximation generated with
`method=DZ2019-MC` ([Deng and Zhang 2019](#ref-DenZha2019approxhv)),
specially with large number of objectives. A more detailed comparison is
provided by López-Ibáñez ([2026](#ref-Lop2026hvapprox)).
...
for (dataset in datasets) {
  ...
  res <- list(
    data.frame(Method="DZ2019-HW", samples = samples, seed = NA,
      hv = sapply(samples, function(i)
        hv_approx(x, reference=ref, nsamples=i, method="DZ2019-HW"))),
    data.frame(Method="Rphi-FWE+", samples = samples, seed = NA,
      hv = sapply(samples, function(i)
        hv_approx(x, reference=ref, nsamples=i, method="Rphi-FWE+"))))
  # Duplicate the observations to avoid a warning with mean_cl_boot
  res <- c(res, res)
  for (k in seq_len(nreps)) {
    seed <- 42 + k
    res <- c(res, list(
      data.frame(Method="DZ2019-MC", samples = samples, seed = seed,
        hv = sapply(samples, function(i)
...
[`hv_approx_fpras()`](https://multi-objective.github.io/moocore/r/reference/hv_approx_fpras.md)
allows obtaining an approximation with relative error smaller than
$`\epsilon`$ (`epsilon`) with a given probability $`1-\delta`$
(`delta`). As the plot below shows, the approximation error is often
better than the requested value, but the computation time increases very
quickly for smaller `epsilon`.
...
for (r in seq_len(nreps)) {
  z <- generate_ndset(npoints, dim, method = shape, seed = 42 + r)
  exact <- hypervolume(z, reference = ref)
  for (epsilon in c(0.1, 0.025, 0.01, 0.005)) {
    for (delta in c(0.25, 0.1, 0.05)) {
      t_start <- proc.time()[["elapsed"]]
      hv <- hv_approx_fpras(z, reference = ref, epsilon = epsilon, delta = delta)
      t_end <- proc.time()[["elapsed"]] - t_start
      res <- rbind(res, data.frame(
        r = r,
        epsilon = epsilon,
        delta = delta,
        hverror = abs(1 - hv / exact),
        time = t_end
      ))
    }
  }
...
ggplot(res, aes(x = as.factor(epsilon), y = hverror, fill = delta)) +
  geom_boxplot() +
  scale_y_log10(labels = label_log()) +
  labs(x = "epsilon", y = "Relative error") +
  ggtitle(paste0(shape, "-", npoints, "-", dim, "d")) +
  theme_bw()
```

![Boxplot of relative error and line plot of CPU time for FPRAS with
varying epsilon and delta.](hv_approx_files/figure-html/fpras-1.png)

``` r


ggplot(res, aes(x = epsilon, y = time, color = delta, group = delta)) +
  stat_summary(fun = mean, geom = "line") +
  stat_summary(fun = mean, geom = "point") +
  scale_y_log10(labels = label_log()) +
  scale_x_log10(breaks = sort(unique(res[["epsilon"]]))) +
  labs(y = "CPU time (s)") +
  ggtitle(paste0(shape, "-", npoints, "-", dim, "d")) +
  theme_bw()
```
...
![Boxplot of relative error and line plot of CPU time for FPRAS with
varying epsilon and delta.](hv_approx_files/figure-html/fpras-2.png)

## References

Deng, Jingda, and Qingfu Zhang. 2019. “Approximating Hypervolume and
Hypervolume Contributions Using Polar Coordinate.” *IEEE Transactions on
Evolutionary Computation* 23 (5): 913–18.
<https://doi.org/10.1109/tevc.2019.2895108>.

López-Ibáñez, Manuel. 2026. “Approximating the Hypervolume Indicator
Using Fast Low-Discrepancy Sequences.” In *Proceedings of the Genetic
and Evolutionary Computation Conference, GECCO 2026*, edited by Ting Hu
and Leonardo Trujillo. ACM Press.
