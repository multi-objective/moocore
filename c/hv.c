...
#define STOP_DIMENSION 3 /* default: stop on dimension 3 */
              ...
              const double * restrict ref)
{
    ...
    for (int k = d-1; k >= 0; k--) {
        ...
        if (j < 0)
            continue;
        qsort(scratch, n, sizeof(dlnode_t*), cmp_dlnode_asc);
...
struct hv4d_state;
double hv4d_recursive(struct hv4d_state * list4d, size_t n, const double * restrict ref);

const double ** hv4d_get_scratch(void *list4d);

static double
fpli_hv4d(dlnode_t *list, struct hv4d_state * list4d, size_t c, const double * restrict ref)
{
    ...
    const double ** data = hv4d_get_scratch(list4d);
    size_t j = 0;
    do {
        data[j] = pp->x;
        j++;
    ...
    assert(c == j);
    assert(c > 1);
    double hv = hv4d_recursive(list4d, c, ref);
    return hv;
}

static double
hv_recursive(dlnode_t * restrict list, void * restrict list4d,
             dimension_t dim, size_t c,
             const double * restrict ref, double * restrict bound);

static void
skip_or_recurse(dlnode_t *p, dlnode_t *list, void * restrict list4d,
                dimension_t dim, size_t c,
                const double * restrict ref, double * restrict bound)
{
    ASSUME(dim > STOP_DIMENSION);
    ...
    if (p->ignore >= dim) {
        ...
        p->area[d_stop] = hv_recursive(list, list4d, dim-1, c, ref, bound);
             ...
             const double * restrict ref, double * restrict bound)
{
    ...
    if ( dim > STOP_DIMENSION ) {
        ...
        skip_or_recurse(p1, list, list4d, dim, c, ref, bound);
...
void * setup_list4d(size_t n, const double * ref);
void free_list4d(void * );
               ...
               const double * restrict ref)
{
    ...
    if (unlikely(n == 0)) {
        ...
        void * list4d = setup_list4d(n, ref);
