...
/**
   ...
   Frontier invariant:
       ...
       x increases strictly from left to right
       y decreases strictly from left to right
...
static inline void
treap_node_init_empty(TreapNode *node, double x, double y)
{
    assert(node != NULL);
    node->x = x;
    node->y = y;
...
*/
static inline void
treap_node_init(TreapNode *node, double x, double y, TreapItem item)
{
    treap_node_init_empty(node, x, y);
    node->item = item;
}

static inline void
treap_init_with_singleton(Treap *tree, TreapNode * node, double x, double y, TreapItem item)
{
    treap_node_init(node, x, y, item);
    treap_init_with_single_node(tree, node);
}
#else
static inline void
treap_node_init(TreapNode *node, double x, double y)
{
    treap_node_init_empty(node, x, y);
}

static inline void
treap_init_with_singleton(Treap *tree, TreapNode * node, double x, double y)
{
    treap_node_init(node, x, y);
    treap_init_with_single_node(tree, node);
}
#endif
...
static inline void
treap_split_lt_with_max(TreapNode *root, double key,
    ...
    if (root->x < key) {
        *left = root;
        if (left_max) {
            ...
            treap_split_lt_with_max(root->right, key, &root->right, right,
                                    &sub_left_max);
            ...
            *left_max = sub_left_max != NULL ? sub_left_max : root;
            ...
            treap_split_lt_with_max(root->right, key, &root->right, right, NULL);
        }
    } else {
        *right = root;
        treap_split_lt_with_max(root->left, key, left, &root->left, left_max);
    }
}
...
/**
   ...
   On return:
       ...
       *left  contains every node with x < key
       *right contains every node with x >= key
...
*/
static inline void
treap_split_lt(TreapNode *root, double key,
               TreapNode **left, TreapNode **right)
{
    treap_split_lt_with_max(root, key, left, right, NULL);
}
...
*/
static inline void
treap_split_le(TreapNode *root, double key,
               TreapNode **left, TreapNode **right)
{
    if (root == NULL) {
        *left = NULL;
    ...
    if (root->x <= key) {
        ...
        *left = root;
        treap_split_le(root->right, key, &root->right, right);
    } else {
        // root belongs to the right result.
        *right = root;
        treap_split_le(root->left, key, left, &root->left);
    }
}
...
/**
   ...
   Since y strictly decreases as x increases:
       ...
       *high_y contains every node with node->y >= y
       *low_y  contains every node with node->y <  y

   Therefore *high_y is a prefix in x order.
...
/**
   ...
   PRECONDITION:
       ...
       Every x in left is less than every x in right.
...
*/
static inline TreapNode *
treap_find_le(const Treap *tree, double key)
{
...
static inline TreapNode **
treap_find_le_link(Treap *tree, double key)
{
...
*/
static inline TreapNode *
treap_find_lt(const Treap *tree, double key)
{
    TreapNode *node = tree->root, *best = NULL;
...
/**
   Find the node with the smallest x such that x >= key.
...
*/
static inline TreapNode *
treap_find_ge(const Treap *tree, double key)
{
    TreapNode *node = tree->root, *best = NULL;
...
*/
static inline TreapNode *
treap_find_gt(const Treap *tree, double key)
{
    TreapNode *node = tree->root, *best = NULL;
...
/**
   Find a node with exactly x == key.
 ...
 */
static inline TreapNode *
treap_find(const Treap *tree, double key)
{
...
/**
   ...
   PRECONDITIONS:
       ...
       - node->x does not already exist in the treap.
       - node->left == NULL.
       - node->right == NULL.
...
treap_insert_node(Treap *tree, TreapNode *node)
{
    ...
    assert(node->left == NULL);
    assert(node->right == NULL);
    // node->x is not already in the tree.
    assert(treap_find(tree, node->x) == NULL);
    ...
    /* Since keys are guaranteed unique by the caller, there is no need to
       search for an existing x before doing the split.  */
    treap_split_lt(tree->root, node->x, &left, &right);
                                     ...
                                     TreapNode **prev, TreapNode **next)
{
    ...
    /* On the frontier, z[1] decreases with increasing x.  Therefore the
