...
#define vector_define(VECTOR_TYPE, BASE_TYPE)                                \
    ...
    struct VECTOR_TYPE {                                                     \
        ...
        BASE_TYPE * _capacity;                                               \
                                                                             ...
                                                                             \
    static inline void VECTOR_TYPE##_reserve(VECTOR_TYPE * v, size_t n);     \
                                                                             \
    /* Default constructor: constructs an empty vector, with no content
   and a size of zero. */      \
    static inline void VECTOR_TYPE##_ctor_zero(VECTOR_TYPE * v)              \
    {                                                                        \
        v->_begin = NULL;                                                    \
        v->_end = NULL;                                                      \
        v->_capacity = NULL;                                                 \
    ...
    /* Default constructor: constructs an empty vector, with no content
   and a size of zero. */      \
    static inline void VECTOR_TYPE##_ctor(VECTOR_TYPE * v, size_t n)         \
    {                                                                        \
   ...
   content and a size of zero. */   \
    static inline VECTOR_TYPE * VECTOR_TYPE##_new(size_t n)                  \
    {                                                                        \
   ...
   does not try to free each of its elements or the vector itself. */       \
    static inline void VECTOR_TYPE##_dtor(VECTOR_TYPE * v)                   \
    {                                                                        \
        free(v->_begin);                                                     \
    }                                                                        \
                                                                             \
   ...
   elements. */            \
    static inline void VECTOR_TYPE##_delete(VECTOR_TYPE * v)                 \
    {                                                                        \
        VECTOR_TYPE##_dtor(v);                                               \
        free(v);                                                             \
    }                                                                        \
    ...
    static inline size_t VECTOR_TYPE##_size(const VECTOR_TYPE * v)           \
    {                                                                        \
        return v->_end - v->_begin;                                          \
    }                                                                        \
                                                                             \
    /* Return size of allocated storage capacity. */                         \
    static inline size_t VECTOR_TYPE##_capacity(const VECTOR_TYPE * v)       \
    {                                                                        \
   ...
   size is 0. */       \
    static inline bool VECTOR_TYPE##_empty(const VECTOR_TYPE * v)            \
    {                                                                        \
   ...
   pointers. */   \
    static inline void VECTOR_TYPE##_push_back(VECTOR_TYPE * v, BASE_TYPE x) \
    {                                                                        \
   ...
   it. Note that the element is not freed. */      \
    static inline BASE_TYPE VECTOR_TYPE##_pop_back(VECTOR_TYPE * v)          \
    {                                                                        \
   ...
   reference like in C++! */     \
    static inline BASE_TYPE VECTOR_TYPE##_front(const VECTOR_TYPE * v)       \
    {                                                                        \
        cvector_assert(!VECTOR_TYPE##_empty(v));                             \
        return v->_begin[0];                                                 \
    }                                                                        \
   ...
   reference like in C++! */      \
    static inline BASE_TYPE VECTOR_TYPE##_back(const VECTOR_TYPE * v)        \
    {                                                                        \
        cvector_assert(!VECTOR_TYPE##_empty(v));                             \
        return *(v->_end - 1);                                               \
    }                                                                        \
   ...
   vector container. */   \
    static inline BASE_TYPE * VECTOR_TYPE##_begin(const VECTOR_TYPE * v)     \
    {                                                                        \
        return v->_begin;                                                    \
    }                                                                        \
                                                                             \
   ...
   in the vector container. */   \
    static inline BASE_TYPE * VECTOR_TYPE##_end(const VECTOR_TYPE * v)       \
    {                                                                        \
        return v->_end;                                                      \
    }                                                                        \
                                                                             \
    /* Returns the element at position pos. */                               \
    static inline BASE_TYPE VECTOR_TYPE##_at(const VECTOR_TYPE * v,          \
                                             size_t pos)                     \
    {                                                                        \
    ...
    static inline BASE_TYPE VECTOR_TYPE##_set(const VECTOR_TYPE * v,         \
                                              size_t pos, BASE_TYPE e)       \
    {                                                                        \
        cvector_assert(pos <= VECTOR_TYPE##_size(v));                        \
