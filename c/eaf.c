...
static int
compare_x_asc(const void * p1, const void * p2)
{
...
static int
compare_y_desc(const void * p1, const void * p2)
{
...
{
    fprintf(stream, point_printf_format point_printf_sep point_printf_format, x,
            y);
}
...
eaf_t *
eaf_create(int nobj, int nruns, int npoints)
{
...
void
eaf_realloc(eaf_t * eaf, size_t nobj)
{
    const int nruns = eaf->nruns;
    eaf->data = realloc(eaf->data, sizeof(objective_t) * nobj * eaf->maxsize);
    assert(eaf->data);
    eaf->bit_attained =
        realloc(eaf->bit_attained, bit_array_bytesize(nruns) * eaf->maxsize);
...
eaf_store_point_help(eaf_t * eaf, int nobj, const int * save_attained)
{
    ...
    if (eaf->size == eaf->maxsize) {
        ...
        eaf->maxsize = (size_t)((double)eaf->maxsize *
                                (1.0 + 1.0 / pow(2, eaf->nreallocs / 4.0)));
    ...
    for (int k = 0; k < nruns; k++) {
        bit_array_set(bit_array_offset(eaf->bit_attained, eaf->size, nruns), k,
                      (bool)save_attained[k]);
    }
...
eaf_print_line(FILE * coord_file, FILE * indic_file, FILE * diff_file,
               const objective_t * x, int nobj, const bit_array * attained,
               int nruns)
{
    if (coord_file) {
...
void
eaf_print_attsurf(const eaf_t * eaf, int nobj, FILE * coord_file,
                  FILE * indic_file, FILE * diff_file)
{
    for (size_t i = 0; i < eaf->size; i++) {
        ...
        eaf_print_line(coord_file, indic_file, diff_file, p, nobj,
...
fprint_set2d(FILE * stream, const objective_t * const * data, int ntotal)
{
    for (int k = 0; k < ntotal; k++)
        fprintf(stream,
                "%6d: " point_printf_format " " point_printf_format "\n", k,
                data[k][0], data[k][1]);
}
             ...
             const double * percentile, int nlevels)
{
    ...
    for (int k = 0; k < nlevels; k++) {
        ...
        double p =
            percentile ? percentile[k] : level2percentile(k + 1, nlevels);
...
void
eaf2matrix(double * rmat, eaf_t * const * eaf, int nobj,
...
double *
eaf_compute_matrix(int * eaf_npoints, double * data, int nobj,
                   const int * cumsizes, int nruns, const double * percentile,
                   int nlevels)
{
      ...
      const int * attlevel, const int nlevels)
{
    ...
    DEBUG2(fprintf(stderr, "Original data:\n");
    ...
    DEBUG2(fprintf(stderr, "Sorted data (x):\n");
           fprint_set2d(stderr, datax, ntotal);
...
eaf_diff_color(const eaf_t * eaf, size_t k, int nruns)
{
    const bit_array * bit_attained =
        bit_array_offset(eaf->bit_attained, k, nruns);
    int count_left, count_right;
    attained_left_right(bit_attained, nruns / 2, nruns, &count_left,
                        &count_right);
...
static void
min_max_in_objective(const objective_t * v, int nobj, int k,
...
static bool
polygon_dominates_any_point(const objective_t * a, const objective_t * b,
                            int nobj)
{
...
polygons_intersect(const objective_t * a, const objective_t * b, int nobj)
{
    ...
    // by a corner of A
    return polygon_dominates_any_point(a, b, nobj) &&
           polygon_dominates_any_point(b, a, nobj);
}
...
#define POLY_SIZE_CHECK()                  \
    do {                                   \
        _poly_size_check--;                \
        assert(_poly_size_check >= 4);     \
        assert(_poly_size_check % 2 == 0); \
        _poly_size_check = 0;              \
    } while (0)
#define eaf_point(A, K) (eaf[(A)]->data + (K) * nobj)
#define push_point_color(X, Y, C)                      \
    do {                                               \
        vector_objective_push_back(&polygon->xy, (X)); \
...
static size_t
rectangle_add(eaf_polygon_t * regions, objective_t lx, objective_t ly,
              objective_t ux, objective_t uy, int color)
{
#if 0
...
#define printf_points(ka, kb, pka, pkb)
        ...
        while (true) {
            ...
            while (pka[1] < pkb[1]) {
                if (pka[0] < pkb[0]) // pka strictly dominates pkb
                    rectangle_add(regions, pka[0], pkb[1], pkb[0], top,
                                  color[ka]);
                top = pkb[1];
                kb++;
                if (kb >= eaf_b_size)
    ...
    close_eaf:
        ...
        while (true) {
            assert(pka[1] < pkb[1]);
            rectangle_add(regions, pka[0], pka[1], objective_MAX, top,
                          color[ka]);
            top = pka[1];
            ka++;
            if (ka >= eaf_a_size)
