...
#define OPTION_VERSION_STR                                                   \
    "     --version       print version number (and compilation flags) and " \
    "exit;\n"
...
#define OPTION_SEED_STR                                                   \
    " -S, --seed=SEED     Seed of the random number generator (positive " \
    "integer).\n"
...
#define OPTIMISED_FOR_STR ""
    ...
    printf("Copyright (C) " CMDLINE_COPYRIGHT_YEARS "\n" CMDLINE_AUTHORS "\n"
           "This is free software, and you are welcome to redistribute it "
           "under certain\n"
           "conditions.  See the GNU General Public License for details. There "
           "is NO   \n"
...
static inline double *
robust_read_point(char * restrict optarg, int * nobj,
...
static void
data_bounds(double ** minimum_p, double ** maximum_p, const double * data,
            int nobj, int rows)
{
    ASSUME(nobj > 1);
    ASSUME(nobj <= 32);
...
static inline char *
m_strcat(const char * a, const char * b)
{
...
str_is_default(bool flag)
{
    return flag ? "(default)" : "";
}

static inline void
set_program_invocation_short_name(char * s)
{
...
parse_cmdline_minmax(const signed char * minmax, const char * optarg,
                     int * nobj_p)
{
    ...
    if (nobj == 0) {
        ...
        errprintf("number of objectives in --obj (%d) and reference set (%d) "
                  "do not match",
                  tmp_nobj, nobj);
        exit(EXIT_FAILURE);
    }
...
static inline void
default_cmdline_handler(int opt)
{
    switch (opt) {
    case 'V': // --version
        version();
