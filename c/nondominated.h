...
force_agree_minimize(const double * restrict points, dimension_t dim,
                     size_t size, const signed char * restrict minmax,
                                ...
                                const bool keep_weakly)
{
    ...
    do {
        ...
        if (prev[0] > pj[0] || prev[1] > pj[1]) {
            ...
            if (prev &&
                prev[1] <= pj[1]) { // pj is dominated by a point in the tree.
                assert(prev[0] <= pj[0]);
                // It cannot be a duplicate because we already handled them above.
                assert(prev[0] != pj[0] || prev[1] != pj[1] ||
                       prev[2] != pj[2]);
                /* Map the order in p[], which is sorted, to the original order
                   in points. */
...
*/
static inline size_t
find_nondominated_set_3d_(const double * points, size_t size, bool * nondom,
                          const bool keep_weakly)
{
...
static inline size_t
find_dominated_point_agree_none(const double * restrict points, dimension_t dim,
                                size_t size,
...
static inline size_t
find_dominated_point_agree_min(const double * restrict points, dimension_t dim,
                               size_t size, const bool keep_weakly)
{
    ASSUME(dim > 3);
    for (size_t k = 1; k < size; k++) {
        for (size_t j = 0; j < k; j++) {
...
static inline size_t
find_dominated_point_agree_max(const double * restrict points, dimension_t dim,
                               size_t size, const bool keep_weakly)
{
    ASSUME(dim > 3);
    for (size_t k = 1; k < size; k++) {
...
static inline size_t
find_nondominated_set_agree_none(const double * restrict points,
...
static inline size_t
find_nondominated_set_agree_min(const double * restrict points, dimension_t dim,
...
static inline size_t
find_nondominated_set_agree_max(const double * restrict points, dimension_t dim,
                                size_t size, bool * restrict nondom,
                                const bool keep_weakly)
{
    ASSUME(dim > 3);
...
**/
static inline size_t
find_dominated_point_(const double * restrict points, dimension_t dim,
                      size_t size, const signed char * restrict minmax,
                      signed char agree, const bool keep_weakly)
{
    ...
    if (dim <= 3) {
        const double * pp =
            force_agree_minimize(points, dim, size, minmax, agree);
        size_t res;
        if (dim == 2) {
    ...
    switch (agree) {
    case AGREE_NONE:
        return find_dominated_point_agree_none(points, dim, size, minmax,
                                               keep_weakly);
    case AGREE_MINIMISE:
        return find_dominated_point_agree_min(points, dim, size, keep_weakly);
    case AGREE_MAXIMISE:
        return find_dominated_point_agree_max(points, dim, size, keep_weakly);
    default:
        unreachable();
    }
}
...
**/
static inline size_t
find_nondominated_set_(const double * restrict points, dimension_t dim,
                       size_t size, const signed char * restrict minmax,
                       signed char agree, bool * nondom, const bool keep_weakly)
{
    ASSUME(minmax != NULL);
    ASSUME(nondom != NULL);
    if (size < 2)
    ...
    switch (agree) {
    case AGREE_NONE:
        return find_nondominated_set_agree_none(points, dim, size, minmax,
                                                nondom, keep_weakly);
    case AGREE_MINIMISE:
        return find_nondominated_set_agree_min(points, dim, size, nondom,
                                               keep_weakly);
    case AGREE_MAXIMISE:
        return find_nondominated_set_agree_max(points, dim, size, nondom,
                                               keep_weakly);
    default:
        unreachable();
    }
...
static inline size_t
find_weakly_dominated_point(const double * restrict points, int dim,
                            size_t size, const bool * restrict maximise)
{
    ASSUME(dim >= 2);
    ASSUME(dim <= 32);
                            ...
                            bool * nondom)
{
    ASSUME(dim >= 2);
    ASSUME(dim <= 32);
    return find_nondominated_set_(points, (dimension_t)dim, size, minmax, agree,
                                  nondom, /* keep_weakly = */ false);
}
                      ...
                      const signed char * minmax, bool * nondom)
{
    ...
    ASSUME(dim <= 32);
    size_t new_size =
        find_nondominated_set_(points, dim, size, minmax, AGREE_NONE, nondom,
                               /* keep_weakly = */ false);

    if (new_size > size || new_size == 0 ||
        new_size == SIZE_MAX) { /* This can't happen.  */
...
static inline size_t
find_weak_nondominated_set(const double * restrict points, dimension_t dim,
                           size_t size, const signed char * restrict minmax,
                           bool * restrict nondom)
{
    ASSUME(dim >= 2);
                                    ...
                                    bool * restrict nondom)
{
    ...
    ASSUME(minmax != NULL);
    size_t new_size =
        find_weak_nondominated_set(points, dim, size, minmax, nondom);
    free((void *)minmax);
    return new_size;
}
...
static inline size_t
get_nondominated_set(double ** pareto_set_p, const double * points, int dim,
                     size_t size, const signed char * minmax)
{
    ASSUME(dim >= 2);
    ASSUME(dim <= 32);
    ASSUME(size > 0);
    ...
    size_t new_size =
        find_nondominated_set(points, (dimension_t)dim, size, minmax, nondom);
...
static inline bool *
is_nondominated_minmax(const double * restrict data, dimension_t nobj,
                       size_t npoint, const signed char * restrict minmax,
                       bool keep_weakly)
{
                         ...
                         bool keep_weakly)
{
    ...
    bool * nondom =
        is_nondominated_minmax(data, nobj, npoint, minmax, keep_weakly);
    free((void *)minmax);
    return nondom;
}
...
static inline void
agree_objectives(double * points, int dim, int size, const signed char * minmax,
                 const signed char agree)
{
    for (int d = 0; d < dim; d++)
...
static inline void
normalise(double * points, int dim, int size, const signed char * minmax,
          signed char agree, const double lower_range, const double upper_range,
...
agree_normalise(double * data, int nobj, int npoint, const bool * maximise,
                ...
                const double * lbound, const double * ubound)
{
    ...
    normalise(data, nobj, npoint, minmax, AGREE_MINIMISE, lower_range,
