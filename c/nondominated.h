                                 ...
                                 const bool keep_weakly)
{
    ...
    for (size_t k = 1; k < size; k++) {
        for (size_t j = 0; j < k; j++) {
            ...
            unsigned nondom_k = (!j_leq_k) | (keep_weakly & k_leq_j);
            // j is removed if it is dominated by k.
            unsigned nondom_j = (!k_leq_j) | j_leq_k;

            /* As soon as j_leq_k and k_leq_j become false, neither k or j will
               be removed, so skip the rest.  */
            if (nondom_k & nondom_j) continue;

            assert(nondom_k ^ nondom_j); // At least one but not both can be removed.
                        ...
                        const bool keep_weakly)
{
    ...
    switch (agree) {
      case AGREE_NONE:
          res = find_nondominated_set_agree_none(
              points, dim, size, minmax, nondom, find_dominated_p, keep_weakly);
          break;
      case AGREE_MINIMISE:
          res = find_nondominated_set_agree_min(
              points, dim, size, nondom, find_dominated_p, keep_weakly);
          break;
      case AGREE_MAXIMISE:
          res = find_nondominated_set_agree_max(
              points, dim, size, nondom, find_dominated_p, keep_weakly);
          break;
      default:
          unreachable();
