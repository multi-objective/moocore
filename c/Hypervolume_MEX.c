...
description(void)
{
    printf(
        ...
        "\nalgorithm achieves O(n^{d-2} log n) time and linear space "
        "complexity "
...
help(void)
{
    ...
    mexPrintf("\nThis function computes the hypervolume value for a set of "
              "points in d-dimension.");
    mexPrintf("\nAll objectives are considered to be minimized.");
    mexPrintf("\nPoints don't have to non-dominated points.");
    mexPrintf("\nThis function takes two arguments, the second is optional");
    mexPrintf("\n\tThe first one is the matrix of points");
    ...
    mexPrintf("\tdisp('This result tends to pi/4=0.7854 when n goes to "
              "infinity');\n\n");
    ...
    mexPrintf("\tdisp('This result tends to 1-pi/6=0.4764 when n goes to "
              "infinity');\n");
    mexPrintf("\tscatter3(XX,YY,ZZ);\n");
}

double *
data_maximum(double * data, int nobj, int rows)
{
    double * vector;
    int n, r, k;
    ...
    */
    if (nrhs == 1) {
        ...
        if (nobj != (mxGetM(prhs[1]) * mxGetN(prhs[1])))
            mexErrMsgTxt("The reference vector should have the same dimension "
                         "any point.");
