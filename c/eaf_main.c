...
usage(void)
{
    ...
    printf("Computes the empirical attainment function (EAF) of all input "
           "FILEs. \n"
           "With no FILE, or when FILE is -, read standard input.\n\n"

           "Options:\n" OPTION_HELP_STR OPTION_VERSION_STR
           " -v, --verbose       print some information (time, input points, "
           "output    \n"
           "                     points, etc) in stderr. Default is --quiet    "
           "        \n"
           " -o, --output FILE   write output to FILE instead of standard "
           "output.      \n"
           ...
           "        \n"
           "                     If FILE is missing use the same file as for "
           "output.   \n"
           " -d[FILE], --diff[=FILE] write difference between half of runs to "
           "FILE.    \n"
           "                     If FILE is '-', print to stdout.              "
           "        \n"
           "                     If FILE is missing use the same file as for "
...
static int
read_doubles(double * vec, char * str)
{
    char * cursor;
    char * endp = str;
    int k = 0;
...
static void
eaf_print(eaf_t ** eaf, int nobj, int nlevels, FILE * coord_file,
...
void
read_input_data(const char * filename, objective_t ** data_p, int * nobjs_p,
                int ** cumsizes_p, int * nsets_p)
{
    int error =
        read_objective_t_data(filename, data_p, nobjs_p, cumsizes_p, nsets_p);
    switch (error) {
    case 0: /* No error */
        break;
...
#define MAX_LEVELS 50
    ...
    while (0 < (option = getopt_long(argc, argv, short_options, long_options,
                                     &longopt_index))) {
        switch (option) {
        case 'l':
    ...
    if (optind < argc) {
        for (k = optind; k < argc; k++) {
            if (strcmp(argv[k], "-"))
                read_input_data(argv[k], &data, &nobj, &cumsizes, &nruns);
            else
